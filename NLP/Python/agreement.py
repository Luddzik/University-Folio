...
def check_node(tr):
	...
	return (matches (V_phrase_num(tr[0]), N_phrase_num(tr[1])) and matches (N_phrase_num(tr[1]), V_phrase_num(tr[2])))
    elif (rule == 'VP -> BE NP'):
	...
	return (matches (N_phrase_num(tr[0]), V_phrase_num(tr[1])))
    #elif (rule == 'S -> WHO QP QM' or rule == 'QP -> VP' or rule == 'VP -> I' or rule == 'VP -> T NP' or rule == 'VP -> BE A' or rule == 'NP -> P' or rule == 'NP -> Nom' or rule == 'Nom -> AN' or rule == 'AN -> N' or rule == 'AN -> A AN' or rule == 'WHO VP' or rule == 'N -> "Ns" etc.'):
	#return True
    else:
	return True
